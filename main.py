...
class ExoplanetClassifierApp:
    ...
    def __init__(self):
        ...
        self.window.app = self  # Reference for callbacks
    ...
    def load_image(self, image_path: str) -> bool:
        ...
        self.window.update_progress(0.1, "Loading image")
        ...
        # Detect graph region
        graph_region = self.image_processor.detect_graph_region()
        
        self.window.update_status("Image loaded successfully")
        self.window.update_progress(0.5, "Ready for analysis")
    ...
    def analyze_image(self, x_range=(0, 1), y_range=(0.95, 1.05)) -> bool:
        ...
        Args:
            x_range: Data range for x-axis
            y_range: Data range for y-axis
        ...
        try:
            ...
            self.window.update_progress(0.5, "Curve extraction")
            ...
            if not self.curve_extractor.reconstruct_light_curve():
                self.window.update_status("Curve reconstruction failed")
                return False
            
            self.window.update_progress(0.65, "Smoothing curve")
            ...
            self.window.update_progress(0.7, "Signal analysis")
            ...
            self.window.update_progress(0.8, "Feature detection")
            ...
            self.confidence, conf_level = self.classifier.calculate_confidence(self.classification)
            
            self.window.update_status(f"Complete! Classification: {primary}")
            self.window.update_progress(1.0, "Complete")
    ...
    def download_and_analyze(self, url: str) -> bool:
        """
        Download image from URL and analyze.
        ...
        Returns:
            bool: True if successful
        """
        self.window.update_status("Downloading image...")
        self.window.update_progress(0.1, "Downloading")
        
        image = Downloader.download_image(url)
        if image is None:
            self.window.update_status("Failed to download image")
            return False
        ...
        return self.load_image(str(temp_path))
    
    def download_from_zooniverse(self, zooniverse_url: str) -> bool:
        """
        Extract and download light curve from Zooniverse subject page.
        
        Args:
            zooniverse_url: Zooniverse subject URL
            
        Returns:
            bool: True if successful
        """
        self.window.update_status("Accessing Zooniverse...")
        self.window.update_progress(0.1, "Fetching page")
        
        image_url = Downloader.extract_zooniverse_image(zooniverse_url)
        if image_url is None:
            self.window.update_status("Could not extract image from Zooniverse page")
            return False
        
        return self.download_and_analyze(image_url)
    ...
    def export_report(self, format_type: str) -> bool:
        ...
        """
        try:
            self.window.update_status(f"Exporting as {format_type.upper()}...")
            ...
            if result:
                self.window.update_status(f"Exported to {result}")
                return True
            else:
                self.window.update_status(f"Export failed")
                return False
        
        except Exception as e:
            logger.error(f"Export error: {str(e)}")
            self.window.update_status(f"Export error: {str(e)}")
            return False
